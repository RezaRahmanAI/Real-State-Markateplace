<template>
  <div>
    <div class="flex items-center mb-2">
      <h6 class="text-15 grow font-bold">List of Gallery</h6>
      <div class="shrink-0">
        <button
      ...
      <tbody>
        ...
        >
          ...
          <td class="px-6 py-4 text-right">
            <button
             @click="showEditModal(item.id,item.title,item.description, item.order,item.contentType)"
...
onMounted(() => {
  nextTick(async () => {
  const { data } = await useUserAPI("/api/gallery", {
    method: "GET",
  });
...
const createData = async  () => {
  ...
  formData.append('contentname', image)
  ...
  const { data } = await useUserAPI("/api/gallery/create", {
    method: "POST",
    body: formData,
  });
  console.log(data);
  ...
  const { data } = await useUserAPI("/api/gallery/edit", {
    method: "POST",
    body: formData,
  });
  console.log(data);
  ...
  const { data } = await useUserAPI("/api/gallery/delete?id="+id+"", {
    method: "POST",
  });
  alert(data.value);
  location.reload(); 
  ...
  const itemActiveInactive=async(id,value)=>{
    console.log(id);
    console.log(value);
    const { data } = await useUserAPI("/api/gallery/itemactiveinactive?id="+id+"&value="+value+"", {
    method: "POST",
  });
  showToatSuccess(data.value);
  location.reload(); 
...
const showEditModal=async(id,title,description,order,contentType)=>{
