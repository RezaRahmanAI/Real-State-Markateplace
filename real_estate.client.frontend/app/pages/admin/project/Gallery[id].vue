<template>
    <div class="">
      ...
      <h6 class="text-15 grow font-bold">Project Gallery</h6>
      <div class="shrink-0">
       <input class="hidden" type="text" id="projectId" name="projectId" :value=" $route.params.id"/>
        <!-- <NuxtLink
          to="/admin/project"
          class="m-2 px-5 py-1 text-white btn bg-orange-600 border-orange-500 hover:text-white hover:bg-orange-600 hover:border-orange-600 focus:text-white focus:bg-orange-600 focus:border-orange-600 focus:ring focus:ring-orange-100 active:text-white active:bg-orange-600 active:border-orange-600 active:ring active:ring-orange-100 dark:ring-orange-400/20"
        >
      ...
      >
        <div class="relative p-4 w-full max-w-2xl h-full">
          ...
          >
            ...
            <form  ref="formRef" @submit.prevent="createData" enctype="multipart/form-data">
        ...
        >
          ...
          <td class="px-6 py-4" >
            <span v-if="item.contentType=='Image'">
              <img class="w-10 h-10 rounded-full" :src="baseUrl + '/api/attachment/get/'+item.content+''" alt="">
            </span>
            <span v-else-if="item.contentType=='Video'">
              <video class="w-10 h-10 rounded-lg" controls>
          ...
          <td class="px-6 py-4">
            <!-- <button
             @click="showEditModal(item.id,item.contentType,item.order)"
...
const projectId = ref('')      // store projectId here, no direct DOM access
const modalRef = ref(null)     // modal DOM ref
const formRef = ref(null)      // form DOM ref
...
// Initialize modal and load data on mount
onMounted(async () => {
  // Set projectId from route params or hidden input, for example:
  ...
  // Wait for next tick and create modal instance
  await nextTick()
  if (modalRef.value) {
    flowbiteModal = new Modal(modalRef.value)
  }
...
async function loadGallery() {
  ...
  const { data } = await useUserAPI("/api/project/getgallery?projectId="+projectId+"", {
    method: "GET",
  });
...
const createData = async  () => {
  ...
  const { data } = await useUserAPI("/api/project/gallerycreate", {
    method: "POST",
    body: formData,
  });
  ...
  const { data } = await useUserAPI("/api/project/gallerydelete?id="+id+"", {
    method: "POST",
  });
  showToatSuccess(data.value);
  // location.reload(); 
  ...
  const itemActiveInactive=async(id,value)=>{
    const { data } = await useUserAPI("/api/project/galleryactiveinactive?id="+id+"&value="+value+"", {
    method: "POST",
  });
  showToatSuccess(data.value);
  // location.reload(); 
