<template>
  <div>
    <div class="flex items-center mb-2">
      <h6 class="text-15 grow font-bold">List of Blogs</h6>
      <div class="shrink-0">
        <button
    ...
    >
      ...
      >
        <tr>
          ...
          <th scope="col" class="px-6 py-3">Offer Date Time</th>
      ...
      <tbody>
        ...
        >
          ...
          <td class="px-6 py-4 text-right">
            <button
             @click="showEditModal(item.id,item.title,item.description, item.postedDate)"
...
onMounted(() => {
  nextTick(async () => {
  const { data } = await useUserAPI("/api/blog", {
    method: "GET",
  });
...
const createData = async  () => {
  ...
  const { data } = await useUserAPI("/api/blog/create", {
    method: "POST",
    body: formData,
  });
  console.log(data);  
  ...
  var postedDate = document.getElementById('editpostedDate').value
  ...
  const { data } = await useUserAPI("/api/blog/edit", {
    method: "POST",
    body: formData,
  });
  console.log(data); 
  ...
  const { data } = await useUserAPI("/api/blog/delete?id="+id+"", {
    method: "POST",
  });
  showToatSuccess(data.value);
  location.reload(); 
  ...
  const itemActiveInactive=async(id,value)=>{
    console.log(id);
    console.log(value);
    const { data } = await useUserAPI("/api/blog/itemactiveinactive?id="+id+"&value="+value+"", {
    method: "POST",
  });
...
const showEditModal=async(id,title,description,postedDate)=>{
    ...
    document.getElementById("editpostedDate").value=postedDate
