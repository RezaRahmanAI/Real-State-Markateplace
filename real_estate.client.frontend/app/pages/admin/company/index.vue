<template>
  <div>
    <div class="flex items-center mb-2">
      <h6 class="text-15 grow font-bold">Company Info</h6>
      <div class="shrink-0">
        <button
    ...
    >
      ...
      >
        <tr>
          <th scope="col" class="px-6 py-3">Phone Title</th>
      ...
      <tbody>
        ...
        >
        <td class="px-6 py-4">{{ item.phoneTilte }}</td>
          ...
          <td class="px-6 py-4 text-right">
            <button
             @click="showEditModal(item.id,item.phoneTilte,item.phone, item.address,item.email,item.latitude,item.longitude)"
...
onMounted(() => {
  ...
  nextTick(async () => {
  const { data } = await useUserAPI("/api/companyinfo", {
    method: "GET",
  });
...
const createData = async  () => {
  ...
  formData.append('phoneTilte', title)
  ...
  const { data } = await useUserAPI("/api/companyinfo/create", {
    method: "POST",
    body: formData,
  });
  ...
  const { data } = await useUserAPI("/api/companyinfo/edit", {
    method: "POST",
    body: formData,
  });
  console.log(data);
  ...
  const { data } = await useUserAPI("/api/companyinfo/delete?id="+id+"", {
    method: "POST",
  });
  showToatSuccess(data.value);
  location.reload(); 
...
const showEditModal=async(id,phoneTilte,phone, address,email,latitude,longitude)=>{
    const $modal= document.querySelector('#editModal');
    const modal = new Modal($modal);
    modal.toggle();
    document.getElementById("editid").value=id
    document.getElementById("edittilte").value=phoneTilte
