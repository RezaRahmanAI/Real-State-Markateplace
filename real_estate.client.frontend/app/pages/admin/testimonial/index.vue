<template>
    <div>
      <div class="flex items-center mb-2">
        <h6 class="text-15 grow font-bold">List of Testimonials</h6>
        <div class="shrink-0">
          <button
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                  ...
                  <div class="sm:col-span-2">
                    <label
                      ...
                      for="file_input"
                    >
                        {{ contentType === 'Video' ? 'Upload file (Video max 15MB)' : 'Upload file' }}
                      </label>
                    <input
                      class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                      aria-describedby="file_input_help"
      ...
      >
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-6 py-4"> <img class="w-16 h-10" :src="baseUrl + '/api/attachment/get/'+item.image+''" alt=""></td>
            ...
            <td class="px-6 py-4 text-right">
              <button
               @click="showEditModal(item.id,item.name,item.description, item.order,item.contentType)"
...
console.log(baseUrl);
  ...
  onMounted(() => {
    nextTick(async () => {
    const { data } = await useUserAPI("/api/testimonial", {
      method: "GET",
    });
  ...
  const createData = async  () => {
    ...
    const { data } = await useUserAPI("/api/testimonial/create", {
      method: "POST",
      body: formData,
    });
    console.log(data);
  ...
  location.reload(); 
    ...
    var type = document.getElementById('editContentType').value
    ...
    const { data } = await useUserAPI("/api/testimonial/edit", {
      method: "POST",
      body: formData,
    });
    console.log(data);
    ...
    const { data } = await useUserAPI("/api/testimonial/delete?id="+id+"", {
      method: "POST",
    });
    alert(data.value);
    }
    const itemActiveInactive=async(id,value)=>{
      console.log(id);
      console.log(value);
      const { data } = await useUserAPI("/api/testimonial/itemactiveinactive?id="+id+"&value="+value+"", {
      method: "POST",
    });
    showToatSuccess(data.value);
  location.reload(); 
  ...
  const showEditModal=async(id,name,description,order,contentType)=>{
