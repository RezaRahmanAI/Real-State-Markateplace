...
onMounted(() => {
  nextTick(() => {
    ...
    if (categorySelect && route.params.category) {
      categorySelect.value = route.params.category;
    }
    getProject();
  });
});
...
const getProject = async () => {
  ...
  const { data } = await useUserAPI(
    `/api/website/getprojects?category=${category}&type=${type}&location=${location}`,
    { method: "GET" }
  );
